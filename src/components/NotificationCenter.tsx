...
import { findAllBudgets, findAllGoals } from "@/services/BudgetService";
...
export function NotificationCenter({dateReference}: GoalProps) {
  const { role } = useAuth();
  ...
  async function checkNotifications() {
    try {
      ...
      const budgets = await findAllBudgets({referenceDate: referenceDate});
      ...
      // Verificar metas
      try {
        const goals = await findAllGoals({referenceDate: dateReference.split("T")[0]});
        
        goals.forEach((goal: Goal) => {
          const current = goal.achievedAmount || 0;
